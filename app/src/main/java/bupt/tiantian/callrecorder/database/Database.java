package bupt.tiantian.callrecorder.database;
...
public class Database extends SQLiteOpenHelper {
    ...
    public static String CREATE_WHITELIST_TABLE = "CREATE TABLE whitelist( _id INTEGER PRIMARY KEY, contact_id TEXT UNIQUE, record INTEGER )";
//    public static String CREATE_WHITELIST_TABLE = "CREATE TABLE whitelist( _id INTEGER PRIMARY KEY, contact_id TEXT, record INTEGER )";
    ...
    public synchronized ArrayList<CallLog> getAllCalls() {
        ...
        try {
            Cursor cursor = db.query(Database.CALL_RECORDS_TABLE,
                    null, null, null, null, null,
                    Database.CALL_RECORDS_TABLE_START_DATE + " DESC");
//          Cursor cursor = db.rawQuery("select * from " + Database.CALL_RECORDS_TABLE, null);
    ...
    public synchronized ArrayList<CallLog> getAllCalls(boolean outgoing) {
        ...
        try {
            Cursor cursor = db.query(Database.CALL_RECORDS_TABLE,
                    null, Database.CALL_RECORDS_TABLE_OUTGOING + "=?",
                    new String[]{outgoing ? "1" : "0"}, null, null,
                    Database.CALL_RECORDS_TABLE_START_DATE + " DESC");
    ...
    public synchronized boolean addWhitelist(Whitelist contact) {
        SQLiteDatabase db = this.getWritableDatabase();
        try {
            db.insertOrThrow(Database.WHITELIST_TABLE, null, contact.getContent());
            return true;
        } catch (SQLException e) {
            return false;
        } finally {
